...
class Model(algorithm.Algorithm):
  ...
  def model_fn(self, features, labels, mode):
    ...
    input_layer = tf.layers.dense(inputs=input_layer, units=512, activation=tf.nn.relu)

    # Dropout with rate = 0.2
    input_layer = tf.layers.dropout(inputs=input_layer, rate=0.2,
        training=mode == tf.estimator.ModeKeys.TRAIN)

    # Dense layer with 256 neurons
    input_layer = tf.layers.dense(inputs=input_layer, units=256, activation=tf.nn.relu)
  ...
  def choose_to_stop_early(self):
    ...
    return self.cumulated_num_steps > 10000
