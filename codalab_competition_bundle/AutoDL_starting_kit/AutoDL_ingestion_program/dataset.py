...
# The module data_pb2 is automatically generated by compiling data.proto
# See more details at:
...
def dataset_file_pattern(dataset_name, set='train'):
  if set == 'train':
    return os.path.join("", dataset_name, "*train*")
  elif set == 'test':
    return os.path.join("", dataset_name, "*test-examples*")
  else:
    raise KeyError("Wrong key: `set` should be equal to 'train' or 'test'.")
...
class AutoDLDataset(object):
  ...
  ################ TODO ################
  # TODO: to be discussed: train/test split
  ...
  def _create_dataset(self):
    if not hasattr(self, "dataset_"):
      ...
      files_training = gfile.Glob(dataset_file_pattern(self.dataset_name_,
                                                       set='train'))
      files_test = gfile.Glob(dataset_file_pattern(self.dataset_name_,
                                                       set='test'))
      if not files_training:
        raise IOError("Unable to find files for training. data_pattern='" +
                      dataset_file_pattern(self.dataset_name_, set='train') +
                      "'.")
      if not files_test:
        raise IOError("Unable to find files for test. data_pattern='" +
                      dataset_file_pattern(self.dataset_name_, set='test') +
                      "'.")

      # logging.info("Number of training files: %s.", str(len(files)))
      logging.info("Number of training files: %s.", str(len(files_training)))
      logging.info("Number of test files: %s.", str(len(files_test)))

      # self.dataset_ = tf.data.TFRecordDataset(files)
      self.training_set_ = tf.data.TFRecordDataset(files_training)
      self.test_set_ = tf.data.TFRecordDataset(files_test)
      self.training_set_ = self.training_set_.map(self._parse_function)
      self.test_set_ = self.test_set_.map(self._parse_function)
